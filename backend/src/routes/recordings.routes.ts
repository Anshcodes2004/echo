...
export async function recordingRoutes(
  app: FastifyInstance,
  options: { service: RecordingService },
) {
  ...
  app.post("/api/recordings", async (request, reply) => {
    ...
    const title =
      input.title ??
      `${input.mode === "meeting" ? "Meeting" : "Personal recording"} — ${new Date().toLocaleDateString("en-CA")}`;
  ...
  app.get("/api/recordings", async (request) => {
    ...
    return {
      recordings: recordings.map(serialize),
      limit: input.limit,
      offset: input.offset,
    };
  });
  ...
  app.get("/api/recordings/:id", async (request, reply) => {
    ...
    const userId = request.userId as string;
    const recording = await service.get(id, userId);
    return recording
      ? serialize(recording)
  ...
  app.patch("/api/recordings/:id", async (request, reply) => {
    ...
    const userId = request.userId as string;
    const recording = await service.rename(id, userId, title);
    return recording
      ? serialize(recording)
      : reply.code(404).send({ error: "Recording not found" });
  });
  ...
  app.post("/api/recordings/:id/finalize", async (request, reply) => {
    const { id } = idSchema.parse(request.params);
    const userId = request.userId as string;
    const recording = await service.finalize(id, userId);
    return recording
      ? serialize(recording)
      : reply.code(404).send({ error: "Recording not found" });
  });
  ...
  app.delete("/api/recordings/:id", async (request, reply) => {
    const { id } = idSchema.parse(request.params);
    const userId = request.userId as string;
    const recording = await service.remove(id, userId);
    return recording
      ? reply.code(204).send()
      : reply.code(404).send({ error: "Recording not found" });
  });
...
function serialize(recording: any) {
  ...
  const { _id, ...rest } = recording.toObject
    ? recording.toObject()
    : recording;
