...
export class TranscriptionGateway {
  ...
  handleUpgrade(request: IncomingMessage, socket: any, head: Buffer) {
    ...
    if (!id || this.active.has(id) || !isAllowedOrigin(request.headers.origin))
    ...
    try {
      ...
      this.server.handleUpgrade(
        ...
        socket,
        head,
        (ws) => void this.open(ws, id, userId),
      );
    } catch {
      return socket.destroy();
    }
  ...
  private async open(ws: WebSocket, id: string, userId: string) {
    const recording = await this.recordings.get(id, userId);
    if (!recording || recording.status !== "recording")
      return ws.close(1008, "Recording is not available for streaming");
    this.active.add(id);
    let stopped = false;
    const finish = async () => {
      if (stopped) return;
      stopped = true;
      await session.close();
      const result = await this.recordings.finalize(id, userId);
      send(ws, {
        type: "completed",
        recordingId: id,
    ...
    const session = new DeepgramLiveSession(
      ...
      {
        onInterim: (segment) =>
          send(ws, { type: "transcript", isFinal: false, ...segment }),
        onFinal: async (segment) => {
          const updated = await this.recordings.appendFinal(
            id,
            userId,
            segment,
          );
        ...
        onError: (error) => {
          send(ws, {
            ...
            code: "DEEPGRAM_ERROR",
            message:
              "Transcription connection failed. Your finalized transcript has been preserved.",
          });
    ...
    ws.on("message", (data, isBinary) => {
      ...
      try {
        ...
        send(ws, {
          type: "error",
          code: "BAD_MESSAGE",
          message: "Unsupported streaming message.",
        });
      }
    });
    ws.on("close", () => {
...
function isAllowedOrigin(origin?: string) {
  return !origin || origin === env.CORS_ORIGIN;
}
